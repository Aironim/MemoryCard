...
def calcRating():
    return('Рейтинг',(window.score/window.total)*100)

def next_question():
    cur_q = randint(0, len(questions_list)-1)
    q = questions_list[cur_q]
    ask(q) 
    print('Статистика')
    print('---Задано всего вопросов---', window.total)
    print('---Верных ответов---', window.score)
    window.total += 1
...
rate = window.score/window.total * 100
